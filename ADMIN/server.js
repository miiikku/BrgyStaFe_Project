...
app.get('/count-processing-blotter', async (req, res) => {
  try {
      const blotterCollection = db.collection('blotter');
      const count = await blotterCollection.countDocuments({ status: 'Processing' });
      res.json({ count });
  } catch (err) {
      console.error('Error fetching count:', err);
      res.status(500).json({ count: 0 });
...
app.get('/count-processing-lupon', async (req, res) => {
  try {
      const luponCollection = db.collection('lupon');
      const count = await luponCollection.countDocuments({ status: 'Processing' });
      res.json({ count });
  } catch (err) {
      console.error('Error fetching count:', err);
      res.status(500).json({ count: 0 });
...
app.get('/count-processing-cfa', async (req, res) => {
  try {
      const cfaCollection = db.collection('cfa');
      const count = await cfaCollection.countDocuments({ status: 'Processing' });
      res.json({ count });
  } catch (err) {
      console.error('Error fetching count:', err);
      res.status(500).json({ count: 0 });
...
app.get('/count-all-processing-complaints', async (req, res) => {
  try {
      const blotterCollection = db.collection('blotter');
      const luponCollection = db.collection('lupon');
      const cfaCollection = db.collection('cfa');

      const [blotterCount, luponCount, cfaCount] = await Promise.all([
      ...
      const totalComplaints = blotterCount + luponCount + cfaCount;
      res.json({ totalComplaints });
  } catch (err) {
      console.error('Error fetching total processing complaints:', err);
      res.status(500).json({ totalComplaints: 0 });
  }
});
